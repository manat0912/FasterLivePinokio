...
class FaceAnalysisModel:
    ...
    def detect_face(self, *data):
        ...
        if self.predict_type == "trt":
            ...
            # Use hardcoded ONNX node names because TRT output order is non-deterministic
            o448 = preds_dict['448'].cpu().numpy()
            o471 = preds_dict['471'].cpu().numpy()
            o494 = preds_dict['494'].cpu().numpy()
            o451 = preds_dict['451'].cpu().numpy()
            o474 = preds_dict['474'].cpu().numpy()
            o497 = preds_dict['497'].cpu().numpy()
            o454 = preds_dict['454'].cpu().numpy()
            o477 = preds_dict['477'].cpu().numpy()
            o500 = preds_dict['500'].cpu().numpy()
            nvtx.range_pop()
        else:
