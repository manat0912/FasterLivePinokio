...
class MotionExtractorModel(BaseModel):
    ...
    def output_process(self, *data):
        # TRT 10 preserves ONNX output order: pitch, yaw, roll, t, exp, scale, kp
        # (TRT 8 used a different order with kp first — that legacy branch is removed)
    ...
    def predict_trt(self, *data):
        ...
        # TRT scrambles output order, must use explicit names
        pitch = preds_dict['pitch'].cpu().numpy()
        yaw = preds_dict['yaw'].cpu().numpy()
        roll = preds_dict['roll'].cpu().numpy()
        t = preds_dict['t'].cpu().numpy()
        exp = preds_dict['exp'].cpu().numpy()
        scale = preds_dict['scale'].cpu().numpy()
        kp = preds_dict['kp'].cpu().numpy()
        
        outs = [pitch, yaw, roll, t, exp, scale, kp]
        nvtx.range_pop()
        return outs
